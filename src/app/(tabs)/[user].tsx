...
export default function Perfil() {
    ...
    const [modalErroVisivel, definirModalErroVisivel] = useState<boolean>(false)
    const id = useLocalSearchParams<{user: string}>()
    ...
    useEffect(() => {
        (async () => {
            if (id) {
                ...
                const adicionarImovel = new Imovel(adicionar, "Adicionar imóvel", "", () => router.navigate("/"));
        ...
        <View className="flex justify-center gap-y-8 p-8 overflow-scroll">
            ...
            { id &&
                <View className="flex flex-row justify-evenly w-full">
                    { editando ?
                        <>
                            <Botao variante="cancelar" onPress={() => {definirEditando(false)}}>
            ...
            { mensagemErro && <Modal titulo={mensagemErro} visible={modalErroVisivel}
            onClose={() => {
              deifnirMensagemErro("")
              definirModalErroVisivel(false)
            }} /> }
        </View>
      </ScrollView>
         :
